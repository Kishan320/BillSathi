  ...
  <div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-semibold text-foreground">{{ $t('purchases.edit_title') }}</h1>
          <p class="text-sm text-muted-foreground">{{ $t('purchases.edit_subtitle') }}</p>
        </div>
      </div>
    </div>

    <!-- Loading -->
    <div v-if="loadingPurchase" class="flex justify-center py-12">
      <AppIcon name="refresh-cw" :size="24" class="animate-spin text-primary" />
    </div>

    <form v-else @submit.prevent="updatePurchase" class="space-y-5">
      <!-- Invoice Details -->
          ...
          <div v-for="(item, index) in form.items" :key="index"
            ...
            <div class="mb-3">
              ...
              <select v-model="item.item_id" @change="selectItem(index, $event)"
                ...
                <option v-for="product in items" :key="product.id" :value="product.id">{{ product.name }}</option>
              </select>
      ...
      <div class="flex justify-end gap-3">
        ...
        <button type="submit" :disabled="loading"
          ...
          {{ loading ? $t('purchases.updating') : $t('common.update') }}
        </button>
      </div>
    </form>
  </div>
...
const form = reactive({ date: '', due_date: '', contact_id: '', warehouse: '', payment_terms: '', notes: '', items: [] });
...
const loadPurchase = async () => {
  try {
    const p = await resourcesStore.getPurchaseById(purchaseId);
    form.date = toDateInput(p.date);
    form.due_date = toDateInput(p.due_date);
    form.contact_id = p.contact_id;
    form.warehouse = p.warehouse || '';
    form.payment_terms = p.payment_terms || '';
    form.notes = p.notes || '';
    form.items = (p.items || []).map(i => ({ item_id: i.item_id, item_name: i.item_name, qty: i.qty, unit_price: i.unit_price, discount: i.discount || 0, taxes: i.taxes || ['GST (18.00%)'], total: i.total }));
  } catch {
    notify.showError(t('common.save_failed'), t('purchases.load_failed'));
...
const updatePurchase = async () => {
  ...
  try {
    await execute(() => resourcesStore.updatePurchase(purchaseId, { ...form, subtotal: totals.value.subtotal, tax: totals.value.tax, total: totals.value.total }));
    notify.showSuccess(t('purchases.edit_title'), t('purchases.updated_success'));
    router.push('/purchases');
  } catch {
    notify.showError(t('common.save_failed'), t('purchases.update_failed'));
  }
};
