  ...
  <div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-muted-foreground">Track and manage expenses</p>
      </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-else-if="!rows.length">
            <td colspan="6" class="text-center py-10 text-muted-foreground">No expenses found.</td>
          </tr>
          <tr v-else v-for="expense in rows" :key="expense.id" class="border-t border-border hover:bg-muted/30 transition-colors">
            <td class="px-4 py-3 text-muted-foreground">{{ formatDate(expense.date) }}</td>
            <td class="px-4 py-3 font-medium text-foreground">{{ expense.reference || '—' }}</td>
            <td class="px-4 py-3 text-muted-foreground">{{ expense.category || '—' }}</td>
            <td class="px-4 py-3 font-semibold text-foreground tabular-nums">{{ formatMoney(expense.amount) }}</td>
            <td class="px-4 py-3">
              <span :class="statusClass(expense.status)" class="px-2 py-0.5 rounded-full text-xs font-medium">
                {{ (expense.status || 'paid').toUpperCase() }}
              </span>
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center justify-end gap-2">
                <button @click="openModal(expense)" type="button" title="Edit"
                ...
                <button @click="deleteExpense(expense)" type="button" title="Delete"
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="w-full max-w-2xl bg-card border border-border rounded-2xl shadow-xl overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 class="text-base font-semibold text-foreground">{{ editId ? 'Edit Expense' : 'New Expense' }}</h2>
...
const rows = computed(() => expensesStore.list);
const meta = computed(() => expensesStore.meta);
...
const expenseSchema = yup.object().shape({
  ...
  status: yup.string().nullable().oneOf(['paid', 'pending', 'overdue']),
...
const openModal = async (expense = null) => {
  editId.value = expense?.id ?? null;
  Object.assign(form, defaultForm(), expense ?? {});
  Object.keys(errors).forEach((k) => delete errors[k]);
...
const saveExpense = async () => {
  ...
  try {
    ...
    if (editId.value) {
      ...
      notify.showSuccess('Updated', 'Expense updated successfully');
    } else {
      await expensesStore.createExpense(payload);
      notify.showSuccess('Created', 'Expense created successfully');
    }
    closeModal();
  } catch (e) {
...
const deleteExpense = async (expense) => {
  if (!confirm(`Delete expense "${expense.reference || expense.id}"?`)) return;
  try {
    await expensesStore.deleteExpense(expense.id);
    notify.showSuccess('Deleted', 'Expense deleted successfully');
  } catch {
    notify.showError('Delete failed', 'Could not delete expense');
  }
};
